...
func (s *testSuite7) TestWindowFunctionsSequential(c *C) {
	...
	tk.MustExec("set @@tidb_window_grouping_concurrency = 1")
	defer func() {
...
func (s *testSuite7) TestWindowFunctionsParallel(c *C) {
	tk := testkit.NewTestKit(c, s.store)
	var result *testkit.Result
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	tk.MustExec("create table t (a int, b int, c int)")
	tk.MustExec("set @@tidb_enable_window_function = 1")
	tk.MustExec("set @@tidb_window_grouping_concurrency = 4")
	defer func() {
		tk.MustExec("set @@tidb_enable_window_function = 0")
		tk.MustExec("set @@tidb_window_grouping_concurrency = 1")
	}()
